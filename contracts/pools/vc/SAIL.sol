...
contract Sail is IVC, PoolWithLPToken, ISwap {
    ...
    //@Todo Fix emission start blocktime
    function emissionStarted() external view returns (bool) {
        return block.timestamp > 1721703784;
    }
    ...
    function initialize() external {
        if (!initialized) {
            PoolWithLPToken._initialize("Sail", "SAIL");
            initialized = true;
        }
    }
    ...
    //@Todo Fix emission rate
    function toEmissionTime(uint256 timestamp) public pure returns (uint256) {
        if (timestamp <= 1721703784) return 0;
        return timestamp - 1721703784;
    }
